...
export const components: Record<string, FC<any>> = {
  h1: (props) => (
    <h1
      className="mb-7 text-balance font-semibold text-neutral-600"
      {...props}
    />
  ),
  h2: (props) => (
    <h2
      className="mt-14 mb-7 text-balance font-semibold text-neutral-600"
      {...props}
    />
  ),
  h3: (props) => (
  ...
  pre: (props) => (
    <pre
      className="relative mt-7 whitespace-pre rounded-sm border border-neutral-950/10 px-4 py-3.5 shadow-xl/5 md:whitespace-pre-wrap"
      {...props}
    />
  ),
  code: async (props) => {
  ...
  hr: (props) => (
    <hr className="my-14 w-24 max-w-xl border-blue-border" {...props} />
  ),
  BlockSideTitle,
  Tool,
  PhotoCaption,
